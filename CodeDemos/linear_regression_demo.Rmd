...
-15.9871 + 0.1448 * 180 #predicted shoesize of a 180cm person
```
...
Tree diameter & volume? Or student height & shoe size?
...
ggplot(survey, aes(x = tattoos, y = sleep)) + 
  geom_point()
...
no_outliers = survey %>% filter(heightin > 20)

ggplot(no_outliers, aes(x = heightcm, y = heightin)) + 
  geom_point() + 
  geom_smooth(method = "lm")

height_lm = lm(heightin ~ heightcm, data = no_outliers)
summary(height_lm)$r.squared
...
ggplot(no_outliers, aes(x = tattoos, y = number)) + 
  geom_point()

model = lm(number ~ tattoos, data = no_outliers)
...
survey$residuals
survey$predicted = predict(surveyline)

ggplot(survey, aes(x=heightcm, y=shoesize)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  geom_point(aes(y = predicted), shape = 1) +
  geom_segment(aes(xend = heightcm, yend = predicted), color = "red")
```
